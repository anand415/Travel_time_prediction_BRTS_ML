...
def quick_hyperopt(data, labels, package='lgbm', num_evals=NUM_EVALS, diagnostic=False):
    ...
    if package=='lgbm':
         ...
         def objective(space_params):
             ...
             cv_results = lgb.cv(space_params, train,num_boost_round=800, nfold = N_FOLDS, stratified=False,
         ...
         space ={'booster' : hp.choice('booster', booster_list),
                  'num_leaves' : hp.quniform('num_leaves', 20, LGBM_MAX_LEAVES, 1),
                 'max_depth': hp.quniform('max_depth', 5, LGBM_MAX_DEPTH, 1),
    ...
    if package=='xgb':
        ...
        space ={'booster' : hp.choice('booster', booster_list),
                ...
                'max_depth': hp.quniform('max_depth', 5, XGB_MAX_DEPTH, 1),
                ...
                'objective' : hp.choice('objective', objective_list),
                'colsample_bytree' : hp.quniform('colsample_bytree', 0.8, 1, 0.1),
                'colsample_bynode' : hp.quniform('colsample_bynode', 0.8, 1, 0.1),
                'colsample_bylevel' : hp.quniform('colsample_bylevel', 0.8, 1, 0.1),
    ...
    if package=='cb':
        ...
        def objective(space_params):
            ...
            cv_results = cb.cv(train, space_params, num_boost_round=500,fold_count=N_FOLDS, 
                             early_stopping_rounds=50, stratified=False, partition_random_seed=42)
        ...
        space ={'depth': 8,
                ...
                'learning_rate' : hp.uniform('learning_rate', 0.2, 0.5),
                ...
                'fold_len_multiplier' : hp.loguniform('fold_len_multiplier', np.log(1.01), np.log(2)),
                'od_type' : 'Iter',
                'od_wait' : 25,
                'task_type' : 'CPU',
                'verbose' : 0
